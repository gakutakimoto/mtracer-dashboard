...
export default function Home() {
  ...
  // SWING TYPE, CLUB TYPE, SUCCESS でフィルタリング
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return matchSwingType && matchClubType && matchSuccess;
    });
  }, [data, selectedSwingType, selectedClubType, selectedSuccess]);
...
<table border="1" style={{ width: "100%", marginBottom: "20px", borderCollapse: "collapse" }}>
  <thead>
    <tr>
      ...
      <th>FACE ANGLE</th>
      <th>Hand First</th> {/* 追加 */}
      <th>Attack Angle</th> {/* 追加 */}
      <th>FacetoPath</th> {/* 追加 */}
      <th>Halfwayback Face Angle</th> {/* 追加 */}
      <th>Down Swing Shaft Rotation</th> {/* 追加 */}
      <th>Top Face Angle</th> {/* 追加 */}
    </tr>
  </thead>
  <tbody>
    {paginatedData.map((row, index) => (
      <tr key={index}>
        ...
        <td>{row.impactHandFirst?.toFixed(2) || "-"}</td> {/* 追加 */}
        <td>{row.impactAttackAngle?.toFixed(2) || "-"}</td> {/* 追加 */}
        <td>{row.impactRelativeFaceAngle?.toFixed(2) || "-"}</td> {/* 追加 */}
        <td>{row.halfwaybackFaceAngleToVertical?.toFixed(2) || "-"}</td> {/* 追加 */}
        <td>{row.downSwingShaftRotationMax?.toFixed(2) || "-"}</td> {/* 追加 */}
        <td>{row.topFaceAngleToHorizontal?.toFixed(2) || "-"}</td> {/* 追加 */}
      </tr>
    ))}
  </tbody>
</table>
